export default function Button({ text = "", variant, size }) {
  const sizeBtn = () => {
    if (size === "lg") {
  ...
  return (
    <a
      href="#"
      className={`${sizeBtn()} ${variantBtn()} tracking-wider font-primary font-bold text-base border-2 rounded-full transition-all`}
    >
      {text}
    </a>
  );
